...
AS
--存货分类
SELECT  'RawMaterial' categoryName , --原材料：RawMaterial 半 / 成品： SemiFinished 类别名称 
...
INNER JOIN  UFDATA_999_2019..mom_orderdetail mos ON mos.MoId = mo.MoId
                                                    AND mos.SortSeq = (select  min(SortSeq) from  UFDATA_999_2019..mom_orderdetail where  mos.MoId = mo.MoId) 
GO
